...
def check_ajax_request_data(data, target_model_name):
    ...
    # 判空
    if not data:
        failed_data['msg'] = '未接受到任何请求数据'
        return json.dumps(failed_data)
    
    # 如果data类型有误
    # 接收类型为字典，ImmutableMultiDict是flask内部实现的字典超类。
    if not (type(data) == dict or type(data) == ImmutableMultiDict):
        failed_data['msg'] = 'AJAX请求数据格式不正确'
        return json.dumps(failed_data)

    model_name = data.get('modelName')
    data_id = data.get('seqid')
    ...
    # 判断data_id空值
    if not data_id:
        failed_data['msg'] = '未指定查询id'
        return json.dumps(failed_data)

    # 判断data_id是否为整形
    if not type(data_id) == int:
        failed_data['msg'] = '查询id不是数字'
        return json.dumps(failed_data)
...
def allowed_file(filename):
    ...
    :return: True or False
    '''
    alloewd_extensions = current_app.config['ALLOWED_EXTENSIONS']
    return '.' in filename and filename[filename.rindex('.') + 1:].lower() in alloewd_extensions

def avioded_file_duplication(filename):
    """
