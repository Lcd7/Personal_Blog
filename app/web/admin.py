import json
import os
from flask import render_template, url_for, redirect, request, abort, current_app, flash, send_from_directory
...
from app.libs.helpers import get_form_error_items, check_ajax_request_data, redirect_back, remove_html_tag, allowed_file, avioded_file_duplication
...
def manage_category():
    ...
    if form.validate_on_submit():
        with db.auto_commit():
            categroy = Category()
            categroy.set_attr(form.data)
            db.session.add(categroy)
        return redirect(url_for('web.manage_category'))

    fields_names, fields_errors = get_form_error_items(form)
    return render_template('admin/manage_category.html', form = form, fields_errors = fields_errors, fields_names = fields_names)
...
def get_category():
    ...
    successful_data['data']['seqid'] = result.seqid
...
def delete_category(category_id):
    ...
    # 判断传递的 id 是不是 1，如果是直接抛出 404 错误
    if category_id == 1:
        abort(404)
    # 接着查询数据库如果没有记录就抛出 404 错误
...
#链接管理视图
@web.route('/admin/link/', methods = ['GET', 'POST'])
@login_required
def manage_link():
    """
    后台链接管理管理视图
    ...
    return render_template('admin/manage_link.html', form = form, fields_errors = fields_errors, fields_names = fields_names)
...
def manage_comment(status):
    ...
    #             comment_id = comment.seqid; action = 'do'
    #         else:
    #             if not comment.from_admin:
    #                 comment_id=comment.seqid; action='undo'
               
    #         model_name='Comment'; record_id=comment.seqid; action='do'
    #     else:
    #         model_name='Comment'; record_id=comment.seqid; action='undo'
    #         model_name='Comment'; record_id=comment.seqid
    ...
    return render_template(
        ...
        comments_info_dict = comments_info_dict,
        status = status
    )
...
@web.route('/admin/trash-record/<any(Comment, Post):model_name>/<int:record_id>/<any(do, undo):action>',  methods=['POST'])
@login_required
def trash_record(model_name, record_id, action):
    ...
    _tempPart = '评论' if model_name == 'Comment' else '文章'
    if action == 'do':
        record.trash = True
        flash_msg = '{}已被移入回收站'.format(_tempPart)
    else:
        record.trash = False
        flash_msg = '{}已被移出回收站'.format(_tempPart)
...
@web.route('/admin/delete-record/<any(Comment, Post):model_name>/<int:record_id>', defaults={'action': 'one'}, methods=['POST'])
@web.route('/admin/delete-record/<any(Comment, Post):model_name>/<any(all, one):action>', defaults={'record_id': None}, methods=['POST'])
@login_required
def delete_record(model_name, record_id, action):
    ...
    if action == 'all':
        with db.auto_commit():
            for record in model.query.filter_by(trash = False).all():
                db.session.delete(record)
        flash('回收站已清空', 'success')
        return redirect_back(default_endpoint = 'web.manage_coment')

    record = model.query.get_or_404(record_id)
    with db.auto_commit():
        db.session.delete(record)
    flash_msg = '评论已删除' if model_name == 'Comment' else '文章已删除'
    flash(flash_msg, 'succes')
    return redirect_back()
...
def manage_post(status):
    ...
    return render_template('admin/manage_post.html',
        pagination = pagination,
        posts_info_dict = posts_info_dict,
        status = status    
    )
...
@web.route('/admin/new-post/', methods = ['GET', 'POST'])
@login_required
def new_post():
    '''
    新建文章视图
    ...
    if form.validate_on_submit():
        #给表单赋值
        form.content.data = request.form.get('markdownEditor-html-code')
        form.categories.data = [Category.query.get(category_id) for category_id in form.categories.data]

        if not form.description.data:
            form.description.data = remove_html_tag(form.content.data)[:150]
...
def edit_post(post_id):
    ...
    if not form.errors:
        ...
        form.categories.data = [category.seqid for category in post.categories]
...
def upload_image():
    ...
    file = request.file.get('editormd-image-file')
    base_info = {
        'success': 0,
        'message': '图片上传失败'
    }
    ...
    # 避免文件名重复
    filename = avioded_file_duplication(file.filename)
    file.save(os.path.join(current_app.config['UPLOAD_FOLDER'], filename))
    base_info['success'] = 1
    base_info['message'] = '图片上传成功'
    base_info['url'] = url_for('web.uploaded_inage', filename = filename)
    return json.dumps(base_info)
...
def blog_setting():
    ...
    if not form.errors:
        ...
        form.post_per_page.data = admin.comment_per_page
